...
const superposition = (cells) => {
	...
	const superMarkersReduce = (masterMarkers, targetSize, pairs) => {
		for (const supers of masterMarkers) {
			const title = pairs ? "Cell Marker Pair" : "Cell Marker";
	...
	const superMarkerValid = (masterMarkers, targetSize, pairs) => {
		for (const supers of masterMarkers) {
			...
			for (let i = supers.length - 1; i >= 0; i--) {
				...
				if (finished) {
					...
					if (validCompletion) {
						...
						const title = `Cell Marker${pairs ? " Pair-" : "-"}`;
						const symbol = 0;
						const cell = null;
			...
			if ((supers.length - splices.length) === 1) {
				...
				const title = `Cell Marker${pairs ? " PairX" : "X"}`;
				const symbol = 0;
				const cell = null;
				results.push(new Result(title, symbol, cell, targetSize));
				return;
	...
	const superSymbolsValid = (masterSymbols, targetSize, pairs) => {
		for (const supers of masterSymbols) {
			...
			if ((supers.length - splices.length) === 1) {
				...
				const title = `Group Symbol${pairs ? " PairX" : "X"}`;
				const symbol = 0;
				const cell = null;
				results.push(new Result(title, symbol, cell, targetSize));
				return;
	...
	superMarkersReduce(masterMarkers2, 2, true);
	if (results.length > 0) {
		for (const result of results) {
			result.cell.delete(result.symbol);
		}
	...
	superSymbolsReduce(masterSymbols2, 2, true);
	if (results.length > 0) {
		for (const result of results) {
			result.cell.delete(result.symbol);
		}
	...
	superMarkersReduce(masterMarkers3, 3, true);
	if (results.length > 0) {
		for (const result of results) {
			result.cell.delete(result.symbol);
		}
	...
	superSymbolsReduce(masterSymbols3, 3, true);
	if (results.length > 0) {
		for (const result of results) {
			result.cell.delete(result.symbol);
		}
